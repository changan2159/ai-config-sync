...
- For Linux-hosted sessions on this machine where Serena and `.NET` are both in play, follow `/home/admin101/projects/2026/ai-config-sync/docs/shared/runtime/linux-serena-dotnet-host-notes.md` instead of inventing alternate host repair paths.
- If a repository uses `.NET` or EF Core, follow `/home/admin101/projects/2026/ai-config-sync/docs/shared/workflows/dotnet-workflow-defaults.md` for CLI tool, restore, and migration defaults instead of improvising per-client behavior.
...
# Cross-Agent Review Policy

- Reuse verified non-interactive review commands instead of improvising new flag combinations each time.
- Prefer the current client's native review path first; only reach for another agent CLI when an independent second pass is actually needed.
- Keep cross-agent review read-only and feed it a fixed patch or commit plus the smallest relevant surrounding context.
- When exact command syntax matters, use the client-specific overlay or repository docs instead of reconstructing flags from memory.
