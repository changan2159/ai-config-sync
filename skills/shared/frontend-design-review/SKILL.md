...
description: >
  Review existing frontend UI implementations, PRs, components, responsive behavior, accessibility, and design-system compliance.
  Evaluates using three pillars: frictionless insight-to-action, quality craft, and trustworthy building.
  USE FOR: PR reviews, design reviews, accessibility audits, design system compliance checks, UI code review,
  component reviews, responsive design checks, and theme testing.
  DO NOT USE FOR: Backend API reviews, database schema reviews, infrastructure or DevOps work, pure business logic
  without UI, non-frontend code, or creating new UI from scratch; use frontend-design for creation.
---

# Frontend Design Review

Review UI implementations against design quality standards and your design system.

## Review Mode

Evaluate existing UI for design system compliance, three quality pillars (Frictionless, Quality Craft, Trustworthy), accessibility, and code quality. Use `frontend-design` when the task is to create or redesign the UI rather than review it.

---

## Creative Direction Review

When reviewing creative UI work, check whether the implementation has a coherent aesthetic direction:
...
Match implementation complexity to vision. Maximalist = elaborate code. Minimalist = restraint and precision. If the task is to create or redesign rather than review, switch to `frontend-design`.
...
**When inspecting an implementation:**
...
3. Check whether the implementation uses design system components + design tokens
