...
Use the platform-specific delegation capability after this skill when a wave should be delegated through a real subagent or sidecar worker.

- Codex: `codex-subagent`
- Pi: `pi-subagents`
- Claude Code: use the Agent tool or the native subagent surface for the current session
- OpenCode: no dedicated delegation skill; use the native model delegation surface or a bounded follow-up prompt
- For the managed cross-client mapping, see `docs/shared/agent-integration.md`

This skill owns the decision about whether parallel work is safe. The platform-specific delegation surface owns the mechanics of the prompt, model choice, read/write boundary, and returned evidence integration.
