...
```bash
python3 scripts/architecture_guardrail.py <repo-path>
```

> **Note:** `scripts/architecture_guardrail.py` is a repo-local script shipped with `ai-config-sync`. Run it from the `ai-config-sync` checkout root; it is not available in arbitrary client repositories.
