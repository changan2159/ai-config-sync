...
description: Builds production-quality UIs. Use when building or modifying user-facing interfaces, components, layouts, state management, hooks, data-flow, and interaction logic. Pair with code-maintainability when adding or changing components, hooks, shared UI patterns, repeated call patterns, or files that risk becoming hard to maintain.
---
...
Use `code-maintainability` alongside this skill when the change adds components, hooks, helpers, data mappings, API calls, form logic, table/list patterns, or repeated UI states. Frontend quality includes not leaving one component file responsible for layout, fetching, normalization, permissions, mutation side effects, and presentation at the same time.

## Component Architecture

Before creating a new component or hook, search for existing design-system primitives, shared components, hooks, data clients, form wrappers, empty/error/loading states, table/list patterns, and route helpers that already express the same concept.

Extract shared UI or logic only when the repeated code represents the same product concept. Keep local helpers local when they serve a single component, and avoid global `utils` or generic hooks that hide unrelated behavior.
